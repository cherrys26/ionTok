...
export class DataService {
    ...
    getVideoList(startIndex: number, chunkSize: number): Observable<Video[]> {
        const start = startIndex;
        const end = startIndex + chunkSize;
    ...
    getVideoResponses(videoId: number) {
        var video = this.videoList.find(x => x.id == videoId);
        
        return of(video.responses);
    }
}
